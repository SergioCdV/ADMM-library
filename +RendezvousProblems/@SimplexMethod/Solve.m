...
function [t, u, e, obj] = Solve(obj)
    ...
    tic
    x0 = pinv(Phi) * b;
    dV = reshape(x0, n, []);             % Initial sequence
    ...
    u = zeros(m,N);
    for i = 1:N
        u(:,i) = Phi(:,1+n*(i-1):n*i) * dV(1:n,i);
    end

    Aeq = u; 
    Idx = Vnorm ~= 0;                         % Non-zero elements
    Aeq(:,Idx) = u(:,Idx) ./ Vnorm(Idx);      % Normal costs

    lb = zeros(1,size(u,2));
    ub = +Inf * ones(1,size(u,2));

    options = optimoptions("linprog", 'Display', 'off');
    sol = linprog(ones(1,size(u,2)), -eye(size(u,2)), zeros(size(u,2),1), Aeq, b, lb, ub, [], options);
